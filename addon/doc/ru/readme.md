...
Также предоставляет дополнительные клавиатурные команды для перемещения между окнами и даёт полезную информацию об eMule.
...
*	control+shift+h: Перемещает фокус и мышь на главную панель инструментов.
*	control+shift+t: Читает текущее окно.
...
*	control+shift+b: Перемещение фокуса в список в текущем окне. Например, используется в окне поиска, загрузки в окне передачи и т.д.
*	control+shift+o: Перемещение фокуса в поля редактирования только для чтения в текущем окне. Например, полученные сообщения IRC, доступные серверы и т.д.
*	control+NVDA+f: Если каретка находится в поле редактирования только для чтения, откроет диалог поиска, доступный в NVDA.
*	control+shift+l: Перемещает объект навигатора и мышь к заголовкам текущего списка.
*	control+shift+q: Читает первый объект в строке состояния; предоставляет информацию о недавней активности.
*	control+shift+w: Читает второй объект в строке состояния; содержит информацию о файлах и пользователях на текущем сервере.
...
Находясь внутри списка, вы можете перемещать каретку между строками и столбцами с помощью комбинаций клавиш alt+control+ Стрелки.
В этом дополнении также доступны следующие сочетания клавиш:
...
*	 Комбинации клавиш, использующихся для перемещения в список результатов поиска, были обобщены, чтобы иметь возможность перемещать фокус на любой доступный список текущего окна.
